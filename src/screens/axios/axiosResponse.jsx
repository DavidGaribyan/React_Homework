      ...
      <ul className="album__list">
        {postCheck &&
          posts.map((post, index) => (
            <li key={`post` + index} onClick={() => userIdHandler(post.id)} className="album__list-item">
              {post.title}
            </li>
          ))}
      </ul>
      <div className="img__wrap">{imgCheck && album.map((album, index) => <img key={`album` + index} src={album.thumbnailUrl} className="img" />)}</div>
    </>
  );
}
