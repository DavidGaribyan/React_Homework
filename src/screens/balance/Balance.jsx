...
export default function Balance() {
  ...
  const balanceData = t('balancePage.balanceData', { returnObjects: true });
  ...
  const filteredData = useMemo(() => {
    switch (balancePeriud) {
      ...
      case '2':
        return balanceData.filter((item) => item.balancePeriud === '1' || item.balancePeriud === '2');
      case '3':
        return balanceData.filter((item) => item.balancePeriud === '1' || item.balancePeriud === '2' || item.balancePeriud === '3');
      default:
        return [];
    }
  ...
  return (
    <>
      <WrapperMain>
        <div>
          <h2 className="balance__heading">{t('balancePage.balanceHeading')}</h2>
        </div>
        <div className="my__balance-wrapper">
          <p className="balance__amount">{t('balancePage.balanceAmount')}</p>
          <Button className="blue balanceBtn" btnName={t('balancePage.balanceBtn')}></Button>
        </div>

        <p className="balanceHistory__heading">{t('balancePage.balanceHistoryListHeading')}</p>
